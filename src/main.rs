...
fn main() {
    ...
    let routes_index = routes_index::RoutesIndex::new(&gtfs);

    let route_id = "434983";
    let route_name = "31";

    let route = routes_index.get_route_by_id(route_id).unwrap();
    println!("{:?}", route);

    let route = routes_index.get_route_by_name(route_name).unwrap();
    println!("{:?}", route);
